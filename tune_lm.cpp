...
int PredictNCELoss(string testfile, int threshold);
...
real alpha = 0.0025, starting_alpha, sample = 0;
real lambda = 0.1, starting_lambda;
real *syn0;
real *syn1, *syn1neg, *expTable, *expPureTable;
//real *A;
...
int evaluateMRRout(int threshold, Paraphrase* pp)
{
    ...
    memcpy(M, syn0, words * layer1_size * sizeof(float));
...
//    }
    ...
    //train_pp_total *= 1;
    //file_size = ftell(fi);
    //PredictNCELoss("/export/a04/moyu/gigaword_data/PPL_eval/nyt.dev", 100000);
    
    for (int ep = 0; ep < 1; ep++) {
        fseek(fi, file_size / (long long)num_threads * (long long)id, SEEK_SET);
        
        while (1) {
            if (word_count - last_word_count >= 100000){ //> 10000) {
                word_actual_old = word_count_actual;
                word_count_actual += word_count - last_word_count; 
                //if (word_count_actual / part == word_actual_old / part + 1) {
                //    evaluateMRRout(10000, ppeval);
                //}
                loss /= (word_count - last_word_count);
                last_word_count = word_count;
                if ((debug_mode > 1)) {
                    now=clock();
                    printf("%cAlpha: %f  Progress: %.2f%%  Loss:%.2lf  Words/thread/sec: %.2fk  ", 13, alpha,
                           word_count_actual / (real)(train_words + 1) * 100, loss,
                           word_count_actual / ((real)(now - start + 1) / (real)CLOCKS_PER_SEC * 1000));
                    fflush(stdout);
                    //PredictNCELoss("/export/a04/moyu/gigaword_data/PPL_eval/nyt.train", 100000);
                    PredictNCELoss("/export/a04/moyu/gigaword_data/PPL_eval/nyt.dev", 100000);
                }
                alpha = starting_alpha * (1 - word_count_actual / (real)(train_words + 1));
                if (alpha < starting_alpha * 0.0001) alpha = starting_alpha * 0.0001;
            ...
            if (pp_count - pp_last_count >= 50000){ //5000){
            ...
            if (cbow) {  //train the cbow architecture
                if(word2vec){
                    ...
                    //loss += target_score;
                    //loss += target_score - log(sum_score);
                }
            }
            sentence_position++;
            if (sentence_position >= sentence_length) {
...
int LoadEmb(string modelname) {
    ...
    for (b = 0; b < words; b++) {
        ...
        //for (a = 0; a < size; a++) fread(&syn0[a + id * size], sizeof(float), 1, f);
        for (a = 0; a < size; a++) fread(&syn0[a + id * size], sizeof(float), 1, f);
        //for (a = 0; a < size; a++) fread(&syn1neg[a + id * size], sizeof(float), 1, f);
        //real len = 0.0;
        //for (a = 0; a < size; a++) len += syn1neg[a + id * size] * syn1neg[a + id * size];
        //len = sqrt(len);
        //for (a = 0; a < size; a++) syn1neg[a + id * size] /= len;
...
void TrainModel() {
    ...
    string initmodel(output_file);
    initmodel += ".init";
    fo = fopen(initmodel.c_str(), "wb");
    if (classes == 0) {
...
int main(int argc, char **argv) {
    ...
    expPureTable = (real *)malloc((EXP_TABLE_SIZE + 1) * sizeof(real));
    for (i = 0; i < EXP_TABLE_SIZE; i++) {
        expPureTable[i] = exp((i / (real)EXP_TABLE_SIZE * 2 - 1) * MAX_EXP); // Precompute the exp() table
    }
...
{
    long long a, c;
    long long last_word, target;
    long long l1, l2;
    double score_total = 0.0;
    double score = 0.0;
    unsigned long long next_random = 3141592654;
    ...
    while (strcmp(line_buf, "")) {
        ...
        while (iss >> word) {
            ...
            int id = SearchVocab((char*)word.c_str());
            words[count] = id;
            count++;
        }
        //score_total = 0;
        ...
        for (int pos = 0; pos < count; pos++) {
            ...
            for (c = 0; c < layer1_size; c++) score += syn1neg[c + l2] * neu1[c];
            //cout << score << endl;
            score_total += log( 1 / (1 + exp(-score)) );
            
            //for (int t = 0; t < 15; t++) {
            for (int t = 0; t < 15; t++) {
                next_random = next_random * (unsigned long long)25214903917 + 11;
                long long wordid = (next_random % threshold) + 1;
                if(wordid == target) continue;
                double sub = 0.0;
                l2 = wordid * layer1_size;
                //cout << wordid << endl;
                for (c = 0; c < layer1_size; c++) sub += syn1neg[c + l2] * neu1[c];
                //cout << sub << endl;
                score_total += log( 1 / (1 + exp(sub)) );
            }
        }
        //printf("%lf\n", score_total);
        ifs.getline(line_buf, 100000, '\n');
    }
    double ppl = score_total / total;
